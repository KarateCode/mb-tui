...
func (m *Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case copyFilesComplete:
		...
		allFiles := linesFromOutput(string(output))
		exts := getRequestedFileExtensions(m.integrationChoice)
		fileNames := filterFilesToIntegration(allFiles, exts)
	...
	case stepEnvMenu:
		m.wwwincEnvMenu, cmd = m.wwwincEnvMenu.Update(msg)
		return m, cmd
	...
	case stepDownloading:
		...
		if m.downloader.Done {
			copyFilesCmd := generateCleanServerFilesCmd(m.batchChoice)
...
func filterFilesToIntegration(allFiles []string, intExts []string) []string {
	var fileNames []string
	for _, file := range allFiles {
		if some(intExts, file) {
...
func some(intExts []string, file string) bool {
	for _, ext := range intExts {
...
func (m *Model) View() string {
	...
	title := lipgloss.Place(
		...
		lipgloss.NewStyle().
			...
			Padding(0, 1).
			Render("  Wwwinc Integration Setup  "),
	)
